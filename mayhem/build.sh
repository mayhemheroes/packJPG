...
for f in $LIBSRCS; do
  o="/tmp/${f%.cpp}.fuzz.o"
  $CXX $SAN_FUZZ $DEBUG_FLAGS $CXXFLAGS -c "$f" -o "$o"
  OBJS="$OBJS $o"
done

# 2) libFuzzer target: in-process harness linked against the engine + the sanitized project lib.
$CXX $SAN_FUZZ $DEBUG_FLAGS $LIB_FUZZING_ENGINE $CXXFLAGS \
...
$CC $SAN_FUZZ $DEBUG_FLAGS -c "$STANDALONE_FUZZ_MAIN" -o /tmp/standalone_main.o
$CXX $SAN_FUZZ $DEBUG_FLAGS $CXXFLAGS \
